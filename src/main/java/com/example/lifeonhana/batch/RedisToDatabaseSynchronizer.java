...
public class RedisToDatabaseSynchronizer {
	...
	public void syncArticleLikesToDatabase() {
		Set<Object> changes = redisTemplate.opsForSet().members("changedArticleLikes");
		...
		changes.parallelStream().forEach(change -> {
			...
			Boolean isLiked = redisTemplate.opsForHash()
				.get("user:" + userId + ":likes", articleId.toString());
			
			// DB 업데이트
			articleLikeRepository.save(new ArticleLike(userId, articleId, isLiked));
		});
